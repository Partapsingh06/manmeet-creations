...
const generateToken = (id) => {
  return jwt.sign({ id }, process.env.JWT_SECRET || 'manmeet_creations_jwt_secret_key_2026', {
    expiresIn: '30d',
  });
};
  ...
  try {
    ...
    if (user) {
      res.status(201).json({
        ...
        message: 'Account registered successfully! Welcome to Manmeet Creations ✨',
      });
    } else {
...
export const addAddress = async (req, res) => {
  try {
    ...
    user.addresses.push({ street, city, state, postalCode, country: country || 'India' });
    await user.save();
...
export const toggleWishlist = async (req, res) => {
  try {
    ...
    res.json({
      ...
      action,
      wishlist: updatedUser.wishlist,
      message: action === 'added' ? 'Added to your Wishlist ❤️' : 'Removed from Wishlist',
    });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
